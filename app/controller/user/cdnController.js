/**
 * @file Used to upload images to cdn service
 *
 * CDN service may includes many cdn vendors. This controller
 * should gather them and convert as a unified API.
 */
...
class CdnController extends Controller {
  ...
  async getImgToken() {
    ...
    const uniqFileName =
      utils.md5sum(fileName + Math.random().toString()) +
      "." +
      fileName.split(".")[1];
    let filePath = `/${page}/${role}/${roleId}${source ? "/" + source : ""}${
      sourceId ? "/" + sourceId : ""
    }/${uniqFileName}`;
  ...
  async showUploadedFiles() {
    ...
    const path = `${savePathPrefix}/${page}/${role}/${roleId}/${source}/${
      sourceId ? sourceId + "/" : ""
    }`;
    ...
    imageFiles = imageFiles
      .concat(fileList.filter(x => x.type === showType))
      .map(x => {
    ...
    if (traversal) {
      // => ['layouts', 'templates']
      let newPath = fileList
        .filter(x => x.type === notShowType)
        .map(x => {
          return x.name;
        });
